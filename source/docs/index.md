...
Due to the amount of code this engine already had and predictably would end up with, the codebase was early on separated into multiple categories. Each category has a general purpose and suggested rules that would be good to follow.
...
In some cases the APIs' are designed with **C** like APIs' focusing on free functions, while some libraries are now utilizing C++ `concepts` and `explicit this` features to utilize member syntax, while allowing to extend types with common functionality. _(See collections library for examples)_
...
- **Collections** - Provides access to basic container types, including but not limited to _String_, _Array_, _HashMap_, and others.
...
- **Tasks** - Provides API interface and implementation for multi-thread processing. _(built around C++ `corotuines`)_
- **DevUI** - Provides API interface for registering of widgets and ImGui utility functions. _(requires runtime implementation)_
