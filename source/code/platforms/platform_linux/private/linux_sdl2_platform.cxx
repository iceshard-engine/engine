...
#include "linux_sdl2_platform.hxx"
...
namespace ice::platform::linux::sdl2
{
    ...
    Platform_LinuxSDL2::Platform_LinuxSDL2(ice::Allocator& alloc) noexcept
    ...
    Platform_LinuxSDL2::~Platform_LinuxSDL2() noexcept
    {
        SDL_QuitSubSystem(SDL_INIT_EVENTS);
        SDL_Quit();
    }

    auto Platform_LinuxSDL2::refresh_events() noexcept -> ice::Result
    {
        IPT_ZONE_SCOPED;
...
namespace ice::platform
{
    ...
    static linux::sdl2::Platform_LinuxSDL2* core_feature;
    static ice::UniquePtr<linux::LinuxStorage> storage_feature;
    static ice::UniquePtr<linux::LinuxThreads> threads_feature;
    ...
    ) noexcept -> ice::Result
    {
        ...
        alignas(alignof(linux::sdl2::Platform_LinuxSDL2)) static char instance_buffer[sizeof(linux::sdl2::Platform_LinuxSDL2)];
        ...
        {
            storage_feature = ice::make_unique<linux::LinuxStorage>(alloc, alloc, params);
        }

        if (ice::has_any(flags, FeatureFlags::StoragePaths))
        {
            threads_feature = ice::make_unique<linux::LinuxThreads>(alloc, alloc, params);
        }

        // Initialize the global platform instance. We don't use the allocator so we don't leak this pointer.
        core_feature = new (instance_buffer) linux::sdl2::Platform_LinuxSDL2{ alloc };
        return S_Success;
    }

    auto query_api(ice::platform::FeatureFlags flag, void*& out_api_ptr) noexcept -> ice::Result
    {
        linux::sdl2::Platform_LinuxSDL2* instance_ptr = core_feature;
    ...
    void shutdown() noexcept
    {
        ...
        if (instance_ptr != nullptr)
        {
            ...
            instance_ptr->~Platform_LinuxSDL2();
