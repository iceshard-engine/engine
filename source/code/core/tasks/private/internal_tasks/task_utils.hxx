...
#if 0
    inline auto internal_process_scheduled_queue(ice::TaskQueue& queue, void* result_ptr, ice::TaskScheduler& scheduler) noexcept
    {
        struct Awaitable
        {
            ice::TaskQueue& queue;
            ...
            inline auto await_suspend(std::coroutine_handle<> coro) noexcept
            {
                ...
                // Prepare a temporary queue for task stealing.
                ice::TaskQueue temp_queue{};
                temp_queue.push_back(tasks_awaitables);

                // Push same number of tasks onto the given scheduler as the number of awaitables.
                std::atomic_uint32_t running = count;
                while(count > 0)
                {
                    ice::schedule_tracked_queue(temp_queue, running, scheduler);
                    count -= 1;
                }

                // Steal a task from the queue if still everything is not processed
                ice::u32 running_val = running.load(std::memory_order_relaxed);
                while(running_val > 0)
                {
                    // Try to process a task, if not possible wait for tasks to finish.
                    if (temp_queue.process_one() == false)
                    {
                        // Wait for the given value.
                        running.wait(running_val, std::memory_order_relaxed);
                    }

                    // Get new value, if == '0' then we are ready to finalize
